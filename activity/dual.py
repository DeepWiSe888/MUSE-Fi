...
# 提取 ResNet-18 的主干网络部分
backbone = torch.nn.Sequential(*(list(pretrained_resnet.children())[:-1]))

# 自定义分类头部，将输出大小改为 194
num_features = pretrained_resnet.fc.in_features
classification_head = torch.nn.Linear(num_features, 194)

# 构建完整的模型
cnn = torch.nn.Sequential(backbone, torch.nn.Flatten(), classification_head)
...
class Net(nn.Module):
    def __init__(self, model = cnn):
        ...
        self.model = model            
        if cfg.data == 'WIFI':
            self.fc = nn.Sequential(*utils.block('BN', 32 * 2, cfg.hid_dim))  
            self.lstm_time = nn.LSTM(32,cfg.hid_dim//2)
            self.lstm_freq = nn.LSTM(32,cfg.hid_dim//2)   
    ...
    def forward(self,x_tmp):
        x_tmp  =  x_tmp.squeeze(dim=1)
        x_tmp =  x_tmp.transpose(1, 2)

        

        """
        x_tmp: sample_size * 193 * 32
        """
        
        bs, win_len_tmp, dim =  x_tmp.shape

        zeros = torch.zeros(bs, 1, dim)
        x = torch.cat([x_tmp, zeros], dim=1)
    
        bs, win_len, dim =  x.shape
        # Time:  sample_size * 194 * 32

        # Freq:  sample_size * 194 * 32
        # x_freq = torch.rfft(x.permute(0,2,1).reshape(-1,win_len),1,onesided=False) 
        x_freq_tmp = torch.fft.fft(x.permute(0, 2, 1).reshape(-1, win_len), dim=1)
        x_freq = torch.stack((x_freq_tmp.real, x_freq_tmp.imag), -1)
       

        x_real_freq = x_freq[:,:,0].reshape(bs,dim,win_len).permute(0,2,1)
        x_img_freq = x_freq[:,:,1].reshape(bs,dim,win_len).permute(0,2,1)
        x_absolute = torch.sqrt((x_real_freq**2) + (x_img_freq**2)) # sample_size * 194 * 32
        ...
        # Cat + FC
        combined = torch.cat([x,x_absolute],-1) # sample_size * 194 * (32*2）
        combined = self.fc(combined) # sample_size * 194 * hid
